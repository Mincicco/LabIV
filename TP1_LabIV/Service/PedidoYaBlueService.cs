using static TP1_LabIV.Models.Models;

namespace TP1_LabIV.Service
{
    public class PedidoYaBlueService
    {
        // Factory method
        public string CreateNewOrder(string type)
        {
            if (type == "blue")
            {
                ...
                return $"Pedido {order.Id} realizado con exito. Dron en camino hacia {order.Address}";
            }
            else
            {
                ...
                return $"Pedido {order.Id} realizado con exito. Delivery en camino hacia {order.Address}";
            }
        }
